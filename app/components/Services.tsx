...
function ServiceCard({ icon, title, description, index, ctaText, currentLocale }: { icon: React.ReactNode; title: string; description: string; index: number; ctaText: string; currentLocale: string }) {
      ...
      <div className="mx-auto max-w-[1320px] px-6 lg:px-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {t.services.items.map((s, i) => (
            <ServiceCard key={s.title} icon={icons[i]} title={s.title} description={s.description} index={i} ctaText={t.services.cta} currentLocale={currentLocale} />
          ))}
        </div>
      </div>
    </section>
